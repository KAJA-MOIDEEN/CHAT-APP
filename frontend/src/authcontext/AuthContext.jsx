...
export const AuthProvider = ({ children }) => {
    ...
    try {
      ...
      if (res.status === 201) {
        return toast.success(res.data.message, {
          style: { backgroundColor: "#994AD2", color: "white" },
        });
      }
  ...
  const signup = async (data) => {
    ...
    try {
      ...
      if (res.status === 201) {
        toast.success(res.data.message, {
          style: { backgroundColor: "#EC4A1C", color: "white" },
        });
        const token = res.data.accessToken;
        setToken(token);
        navigate("/");
    ...
    if (file) {
      try {
        ...
        const res = await axios.post(
          `${backendUrl}/api/auth/profilePic`,
          formData,
          {
            headers: {
  ...
  const getMessages = useCallback(
    async (id) => {
      ...
      try {
        setMsgLoading(true);
        const res = await axios.get(`${backendUrl}/api/message/get/${id}`, {
          withCredentials: true,
        });
        if (res.status === 200) {
    ...
    async (id, token) => {
      ...
      try {
        ...
        const response = await axios.get(
          `${backendUrl}/api/auth/${id}/verify/${token}`
        );
